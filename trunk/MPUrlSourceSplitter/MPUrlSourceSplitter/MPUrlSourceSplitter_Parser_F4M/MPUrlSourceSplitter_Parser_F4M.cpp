        ...
        if (document != NULL)
        {
          ...
          else
          {
            ...
            if (child != NULL)
            {
              ...
              if (declaration != NULL)
              {
                // we have XML declaratio it is valid XML file, just not complete
                this->logger->Log(LOGGER_WARNING, L"%s: %s: XML file probably not complete, XML parse error: %d", PARSER_IMPLEMENTATION_NAME, METHOD_PARSE_MEDIA_PACKET_NAME, parseError);
