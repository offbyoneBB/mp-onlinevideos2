...
CParameter::CParameter(const wchar_t *name, const wchar_t *value)
{
  ...
  unsigned int nameLength = wcslen(name) + 1;
  unsigned int valueLength = wcslen(value) + 1;

  // allocate memory for name and value
  this->name = ALLOC_MEM_SET(this->name, wchar_t, nameLength, 0);
  this->value = ALLOC_MEM_SET(this->value, wchar_t, valueLength, 0);
  ...
  else
  {
    ...
    wcscpy_s(this->name, nameLength, name);
    wcscpy_s(this->value, valueLength, value);
  }
}
