...
bool CCurlInstance::Initialize(void)
{
  ...
  if (result)
  {
    ...
    if (errorCode == CURLE_OK)
    {
      ...
      this->logger->Log(LOGGER_VERBOSE, L"%s: %s: librtmp connection string: %s", this->protocolName, METHOD_START_RECEIVING_DATA_NAME, connectionString);
    ...
    if (errorCode == CURLE_OK)
    {
      ...
      if (errorCode != CURLE_OK)
      {
        this->ReportCurlErrorMessage(LOGGER_ERROR, this->protocolName, METHOD_START_RECEIVING_DATA_NAME, L"error while setting RTMP protocol log callback", errorCode);
        result = false;
      }
    }
    ...
    if (errorCode == CURLE_OK)
    {
      ...
      if (errorCode != CURLE_OK)
      {
        this->ReportCurlErrorMessage(LOGGER_ERROR, this->protocolName, METHOD_START_RECEIVING_DATA_NAME, L"error while setting RTMP protocol log callback user data", errorCode);
        result = false;
      }
    }
