...
HRESULT CAsyncRequest::Request(unsigned int requestId, CAsyncSourceStream *stream, LONGLONG position, LONG length, IMediaSample *sample, BYTE *buffer, DWORD_PTR userData)
{
  ...
  HRESULT result = S_OK;
  CHECK_POINTER_DEFAULT_HRESULT(result, this->stream);
...
void CAsyncRequest::Cancel(void)
{
  this->requestState = CAsyncRequest::Cancelled;
}
