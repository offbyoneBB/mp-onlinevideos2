        ...
        public string GetOtherAsString()
        {
            ...
            if (Other.GetType().IsSerializable)
            {
                try
                {
                    ...
                    new XmlSerializer(Other.GetType()).Serialize(new System.IO.StringWriter(sb), Other);
                    var serialized = string.Format("Serialized://{0}|{1}", Regex.Replace(Other.GetType().AssemblyQualifiedName, @",\sVersion=[^,]+", ""), sb);
                    return serialized;
                }
                catch (Exception ex)
                {
            ...
            if (!string.IsNullOrEmpty(other))
            {
                if (other.StartsWith("Serialized://"))
                {
                    try
                    {
                        ...
                        {
                            Other = new XmlSerializer(resolvedType).Deserialize(new System.IO.StringReader(data));
                            return;
                        }
                    }
                    catch (Exception ex)
