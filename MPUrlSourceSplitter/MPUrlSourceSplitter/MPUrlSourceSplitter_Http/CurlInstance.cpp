...
bool CCurlInstance::Initialize(void)
{
  ...
  if (result)
  {
    ...
    if (errorCode != CURLE_OK)
    {
      this->ReportCurlErrorMessage(LOGGER_ERROR, this->protocolName, METHOD_OPEN_CONNECTION_NAME, L"error while setting follow location", errorCode);
      result = false;
    } 
    ...
    if (errorCode == CURLE_OK)
    {
      ...
      if (errorCode != CURLE_OK)
      {
        this->ReportCurlErrorMessage(LOGGER_ERROR, this->protocolName, METHOD_OPEN_CONNECTION_NAME, L"error while setting debug callback", errorCode);
        result = false;
      }
    }
    ...
    if (errorCode == CURLE_OK)
    {
      ...
      if (errorCode != CURLE_OK)
      {
        this->ReportCurlErrorMessage(LOGGER_ERROR, this->protocolName, METHOD_OPEN_CONNECTION_NAME, L"error while setting debug callback data", errorCode);
        result = false;
      }
    }
    ...
    if (errorCode == CURLE_OK)
    {
      ...
      if (errorCode != CURLE_OK)
      {
        this->ReportCurlErrorMessage(LOGGER_ERROR, this->protocolName, METHOD_OPEN_CONNECTION_NAME, L"error while setting verbose level", errorCode);
        result = false;
      }
    }
    ...
    if (errorCode == CURLE_OK)
    {
      if (!IsNullOrEmpty(this->referer))
      {
        ...
        if (errorCode != CURLE_OK)
        {
          this->ReportCurlErrorMessage(LOGGER_ERROR, this->protocolName, METHOD_OPEN_CONNECTION_NAME, L"error while setting referer", errorCode);
          result = false;
        }
    ...
    if (errorCode == CURLE_OK)
    {
      if (!IsNullOrEmpty(this->cookie))
      {
        ...
        if (errorCode != CURLE_OK)
        {
          this->ReportCurlErrorMessage(LOGGER_ERROR, this->protocolName, METHOD_OPEN_CONNECTION_NAME, L"error while setting cookie", errorCode);
          result = false;
        }
        FREE_MEM(curlCookie);
      }
    ...
    if (errorCode == CURLE_OK)
    {
      if (!IsNullOrEmpty(this->userAgent))
      {
        ...
        if (errorCode != CURLE_OK)
        {
          this->ReportCurlErrorMessage(LOGGER_ERROR, this->protocolName, METHOD_OPEN_CONNECTION_NAME, L"error while setting user agent", errorCode);
          result = false;
        }
    ...
    if (errorCode == CURLE_OK)
    {
      ...
      if (errorCode != CURLE_OK)
      {
        this->ReportCurlErrorMessage(LOGGER_ERROR, this->protocolName, METHOD_OPEN_CONNECTION_NAME, L"error while setting HTTP version", errorCode);
        result = false;
      }
    }
    ...
    if (errorCode == CURLE_OK)
    {
      ...
      if (errorCode != CURLE_OK)
      {
        this->ReportCurlErrorMessage(LOGGER_ERROR, this->protocolName, METHOD_OPEN_CONNECTION_NAME, L"error while setting ignore content length", errorCode);
        result = false;
      }
    }
    ...
    if (errorCode == CURLE_OK)
    {
      ...
      this->logger->Log(LOGGER_VERBOSE, L"%s: %s: requesting range: %s", this->protocolName, METHOD_OPEN_CONNECTION_NAME, range);
      char *curlRange = ConvertToMultiByte(range);
      errorCode = curl_easy_setopt(this->curl, CURLOPT_RANGE, curlRange);
      if (errorCode != CURLE_OK)
      {
        this->ReportCurlErrorMessage(LOGGER_ERROR, this->protocolName, METHOD_OPEN_CONNECTION_NAME, L"error while setting range", errorCode);
        result = false;
      }
      FREE_MEM(curlRange);
      FREE_MEM(range);
