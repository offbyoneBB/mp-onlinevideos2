...
bool CMovieExtendsBox::GetBox(uint8_t **buffer, uint32_t *length)
{
...
uint64_t CMovieExtendsBox::GetBoxSize(uint64_t size)
{
...
bool CMovieExtendsBox::ParseInternal(const unsigned char *buffer, uint32_t length, bool processAdditionalBoxes)
{
  ...
  if (result)
  {
    if (wcscmp(this->type, MOVIE_EXTENDS_BOX_TYPE) != 0)
    {
      // incorect box type
      this->parsed = false;
    }
