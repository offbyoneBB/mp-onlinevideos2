    ...
    public class TV4Play : SiteUtilBase
	{
        ...
        string GetUrlForCategory(string category)
        {
            ...
            if (!Uri.IsWellFormedUriString(category, UriKind.Absolute))
            {
                ...
                {
                    uri = new UriBuilder(baseUrl + "/search" ) { Query = category.Substring(1) }.Uri;
                }
                else
                {
                    if (Uri.TryCreate(new Uri(baseUrl), category + "/search", out uri))
                    {
            ...
            return uri.ToString() + (string.IsNullOrEmpty(uri.Query) ? "?" : "&") + "per_page=999&page=1&is_premium=false&content-type=a-o";
        }
		...
		public override int DiscoverSubCategories(Category parentCategory)
		{
			...
			if (!string.IsNullOrEmpty(data))
			{
                ...
                var a_s = htmlDoc.DocumentNode.Descendants("a");
                if (a_s != null)
                {
                    foreach (var a in a_s)
                    {
                        ...
                        if (idIndex < url.Length)
                        {
                            ...
                            cat.Name = HttpUtility.HtmlDecode(a.InnerText);
