...
namespace OnlineVideos.Sites
{
  ...
  {
    List<string> videosToSkip = new List<string>();
          ...
          try
          {
            ...
            while (m.Success)
            {
...
)
              {
                ...
                Log.Debug("TITLE:" + m2.Groups["gameName"].Value.ToString().ToLower());
    ...
    public override List<SearchResultItem> Search(string query, string category = null)
    {
      ...
      searchUrl = string.Format("http://www.gametrailers.com/feeds/search/child/{0}/?keywords={1}&tabName=videos&platforms=&sortBy=most_recent", promotionID, query);
