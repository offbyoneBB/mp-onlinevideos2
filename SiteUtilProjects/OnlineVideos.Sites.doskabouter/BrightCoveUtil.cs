...
namespace OnlineVideos.Sites
{
    public class BrightCoveUtil : GenericSiteUtil
    {
        ...
        private Dictionary<string, string> getPlaybackOptions(string url)
        {
            ...
            AMF3Object req = MakeRequestsnag("http://www.snagfilms.com/films/title/harmony_oaks/", 101546301001, 867777310001);
            AMF3Serializer ser = new AMF3Serializer();
            byte[] data = ser.Serialize(req, "dfd19675195b031ac05f6b4cca398b291fe0664b");


            //ztele:
            /*
            AMF3Object req = MakeRequest("http://www.ztele.com/webtele/dans-ma-cour-12743/", 792865933001, -1);
            AMF3Serializer ser = new AMF3Serializer();
            byte[] data = ser.Serialize(req, "042b01bed6e20532d32865e198a9f47c6ee4e279");
            */
            using (Stream s = new FileStream(@"E:\new.txt", FileMode.Create, FileAccess.Write))
            {
                BinaryWriter sw = new BinaryWriter(s);
                sw.Write(data);
            }

            AMF3Object response = GetResponse(url, data);
            //Stream stream = new FileStream(@"E:\ztele.txt", FileMode.Open, FileAccess.Read);
            //AMF3Deserializer des = new AMF3Deserializer(stream);
            //AMF3Object response = des.Deserialize();
            ...
            for (int i = 0; i < renditions.Count; i++)
            {
                ...
                string nm = String.Format("{0}x{1} {2}K",
                    rendition.GetIntProperty("frameWidth"), rendition.GetIntProperty("frameHeight"),
                    rendition.GetIntProperty("encodingRate") / 1024);
                result.Add(nm, HttpUtility.UrlDecode(rendition.GetStringProperty("defaultURL")));
            }

            return result;
        }

        private AMF3Object MakeRequestsnag(string referer, long experienceId, long contentId)
        {

            AMF3Object contentOverride = new AMF3Object("com.brightcove.experience.ContentOverride");
            contentOverride.Add("contentId", (double)contentId);
            contentOverride.Add("contentType", 0);
            contentOverride.Add("contentRefId", null);
            contentOverride.Add("featuredRefId", null);
            contentOverride.Add("contentRefIds", null);
            ...
            AMF3Object ViewerExperienceRequest = new AMF3Object("com.brightcove.experience.ViewerExperienceRequest");
            ViewerExperienceRequest.Add("playerKey", String.Empty);
            ViewerExperienceRequest.Add("experienceId", (double)experienceId);
            ViewerExperienceRequest.Add("TTLToken", String.Empty);
            ViewerExperienceRequest.Add("deliveryType", double.NaN);
            ViewerExperienceRequest.Add("contentOverrides", array);
            ViewerExperienceRequest.Add("URL", referer);

            return ViewerExperienceRequest;
        }

        private AMF3Object MakeRequest(string referer, long experienceId, long contentId)
        {
            AMF3Object obj = new AMF3Object("com.brightcove.experience.ViewerExperienceRequest");
            obj.Add("playerKey", "AQ~~,AAAAnreg4lk~,iutjDgKpeknlx9PrvSDXvaNgtPFQB9Vx");
            obj.Add("experienceId", (double)experienceId);
            obj.Add("TTLToken", String.Empty);
            obj.Add("deliveryType", double.NaN);

            AMF3Object obj2 = new AMF3Object("com.brightcove.experience.ContentOverride");
            obj2.Add("contentRefId", "CTAVideoWeb-PK-12743");
            obj2.Add("featuredRefId", null);
            obj2.Add("contentType", 0);
            obj2.Add("featuredId", double.NaN);
            obj2.Add("contentIds", null);

            obj2.Add("contentRefIds", null);
            obj2.Add("target", "videoPlayer");
            obj2.Add("contentId", double.NaN);
            AMF3Array array = new AMF3Array();
            array.Add(obj2);
            obj.Add("contentOverrides", array);
            obj.Add("URL", referer);
            return obj;
        }
